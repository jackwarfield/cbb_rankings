...
int main(int argc, char *argv[]) {
  ...
  loop_schedule("sched.csv", "teams.csv", "teams_2023_rankings.csv", 1, true);
...
void loop_schedule(std::string sched_fn, std::string teams_fnin,
  ...
  std::vector<std::string> home_teams = sched.GetColumn<std::string>("team");
  std::vector<std::string> away_teams =
      sched.GetColumn<std::string>("opponent");
  // std::vector<int> home_id = sched.GetColumn<int>("home_id");
  // std::vector<int> away_id = sched.GetColumn<int>("away_id");
  std::vector<double> home_points = sched.GetColumn<double>("teamscore");
  std::vector<double> away_points = sched.GetColumn<double>("oppscore");
  ...
  for (int _ = 0; _ < n_loops; _++) {
    ...
    for (int i = 0; i < home_teams.size(); i++) {
      ...
      if (h_ind < 0) {
        ...
        ratings.push_back(200.0);
        conferences.push_back("");
        levels.push_back("non-d1");
        rds.push_back(200.0);
        vols.push_back(0.06);
        wins.push_back(0);
        losses.push_back(0);
  ...
  outcsv << "school,id,conference,level,rating,rd,volatility,wins,"
            "losses\n";
  for (int i = 0; i < schools.size(); i++) {
    outcsv << schools[i] << "," << ids[i] << "," << conferences[i] << ","
           << levels[i] << "," << ratings[i] << "," << rds[i] << "," << vols[i]
           << "," << wins[i] << "," << losses[i] << "\n";
  }
  outcsv.close();
}
